...
async function fetchAmazonMeta(amazonUrl) {
  ...
  try {
    ...
    // Extract ASIN: try URL path first, then page HTML (handles promo/deals pages)
    const asin = finalUrl.match(/\/dp\/([A-Z0-9]{10})/i)?.[1]
      || asinFromRedirect
      || html.match(/["']asin["']\s*:\s*["']([A-Z0-9]{10})["']/i)?.[1]
      || html.match(/data-asin="([A-Z0-9]{10})"/)?.[1]
      || html.match(/\/dp\/([A-Z0-9]{10})/i)?.[1]
      || null;
...
function extractAmazonUrls(text) {
  const patterns = [
    ...
    // Promo/deals/coupon pages — these contain product grids with extractable ASINs
    /https?:\/\/(?:www\.)?amazon\.com\/(?:deals|promotion|coupon|b|s)[?\/][^\s"'<>]*/gi,
  ];
  const urls = [];
