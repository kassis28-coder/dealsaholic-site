...
// Fetch Amazon product metadata (title + image) from product page
async function fetchAmazonMeta(amazonUrl) {
  try {
    ...
    // Try to extract price from page HTML
...
export default async (req, context) => {
  ...
  // --- Detect if body is JUST an Amazon URL (Make.com Text Parser sends this) ---
  const isUrlOnly = uniqueUrls.length > 0 && plainText.replace(/https?:\/\/[^\s]+/g, '').trim().length < 10;

  // --- Fetch Amazon metadata when we only have a URL (no title/price in body) ---
  let amazonMeta = null;
  if (isUrlOnly && primaryUrl && !claudeData) {
    amazonMeta = await fetchAmazonMeta(primaryUrl);
  }

  // --- Deal fields: Claude > Amazon meta > regex extraction ---
  ...
  for (const dealUrl of urlsToProcess) {
    ...
    // Image: prefer Amazon meta og:image, fall back to CDN ASIN pattern
    const imageUrl = (urlsToProcess.length === 1 && amazonMeta?.image)
      ? amazonMeta.image
      : dealAsin
    ...
    const submission = {
      id, title: dealTitle, price: price || null,
      originalPrice: originalPrice || null, discount: discount || null,
      url: dealAffiliateUrl, imageUrl,
...
export const config = { path: "/api/submit-email-deal" };import { getStore } from "@netlify/blobs";
