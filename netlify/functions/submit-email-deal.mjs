...
function extractAmazonUrls(text) {
  const patterns = [
    ...
    /https?:\/\/(?:www\.)?amazon\.com\/(?:promocode|gc-apply|promotion)[\/\?][^\s"'<>]*/gi,
...
// Returns true if the URL is a promo/coupon application link, not a product page
function isPromoOnlyUrl(url) {
  return /\/promocode\/|\/gc-apply\/|\/promotion\//i.test(url)
    || /amazon\.com\/coupons/i.test(url);
}

// Extracts a promo code string from a promo URL path or query params
function extractPromoCodeFromUrl(url) {
  const fromPath = url.match(/\/promocode\/([A-Z0-9\-]+)/i)?.[1];
  if (fromPath) return fromPath;
  try {
    const u = new URL(url);
    return u.searchParams.get('promo_code')
...
// Strips email template label noise from a product title
function cleanTitle(title) {
  ...
  title = title.replace(/\s+[A-Z0-9]{4,20}$/, '');

  // Amazon suffix cleanup
  title = title
...
// Extracts deal price and original price from email plaintext
function extractPricesFromText(text) {
  // Match "Deal price: $X" or "Prime Deal Price: $X" — colon required to prevent false positives.
  // (?:prime\s+)? makes the "Prime " prefix optional so both labels are covered by one pattern.
  // If "Deal price: N/A" appears first the regex fails at that position and the engine
  // continues scanning, finding "Prime Deal Price: $X" further in the string.
  const dealMatch = text.match(
    /(?:prime\s+)?deal\s+price\s*:\s*\$?([\d,.]+)/i
  );
  // Match "Original price: $X" — colon required.
  const origMatch = text.match(
    /original\s+price\s*:\s*\$?([\d,.]+)/i
  );

  const dealPrice    = dealMatch ? '$' + parseFloat(dealMatch[1].replace(/,/g, '')).toFixed(2) : null;
  const originalPrice = origMatch ? '$' + parseFloat(origMatch[1].replace(/,/g, '')).toFixed(2) : null;

  if (dealPrice || originalPrice) return { dealPrice, originalPrice };

  // Fallback: collect all dollar amounts — lowest = deal, highest = original
  const allPrices = [...text.matchAll(/\$([\d,.]+)/g)]
    .map(m => parseFloat(m[1].replace(/,/g, '')))
    .filter(p => p >= 0.99 && p < 10000);

  if (allPrices.length >= 2) {
    const sorted = [...new Set(allPrices)].sort((a, b) => a - b);
    // Only split into deal/original if there's a meaningful gap (>5%)
    if (sorted[sorted.length - 1] / sorted[0] > 1.05) {
      return {
        dealPrice:     '$' + sorted[0].toFixed(2),
        originalPrice: '$' + sorted[sorted.length - 1].toFixed(2),
      };
    }
    return { dealPrice: '$' + sorted[0].toFixed(2), originalPrice: null };
  }

  return {
    dealPrice:     allPrices.length ? '$' + allPrices[0].toFixed(2) : null,
    originalPrice: null,
  };
}
...
export default async (req, context) => {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      ...
      // Handle GAS-style payload: { htmlBody, textBody, subject }
      if (parsed.htmlBody || parsed.html) emailBody = parsed.htmlBody || parsed.html;
      if (parsed.textBody || parsed.text) emailBody = emailBody || parsed.textBody || parsed.text;
      // Legacy: wrapped emailBody
      if (parsed.emailBody) {
        emailBody = emailBody || parsed.emailBody;
        try {
  ...
  // After JSON unwrapping, emailBody now holds the actual HTML/text content
  const htmlContent = emailBody;
  const plainText   = stripHtml(htmlContent) || stripHtml(content);

  const allUrls = [];
  if (claudeData?.amazonUrl) allUrls.push(claudeData.amazonUrl);
  extractAmazonUrls(htmlContent).forEach(u => allUrls.push(u));
  ...
  const primaryUrl = productUrls[0] || null;
  ...
  const urlsToProcess = productUrls.length > 0 ? productUrls.slice(0, 20) : [null];
  const savedIds    = [];
  const deals       = [];
  ...
  return new Response(JSON.stringify({
    ...
    ids:             savedIds,
    deals,
    amazonUrlsFound: productUrls.length,
    promoUrlsFound:  promoUrls.length,
    discountCode,
    telegramMessage,
    facebookMessage,
