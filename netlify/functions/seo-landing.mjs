...
export default async function handler(req, context) {
  ...
  const landingDeals = (feed.deals || []).filter(isSeoEligible).filter(landing.filter);
  const matchingDeals = selectedCategory === "all" ? landingDeals : landingDeals.filter(deal => categoryMatch(deal, selectedCategory));
  const deals = sortDeals(matchingDeals, selectedSort).slice(0, 60);
  ...
  <div class="grid">${deals.length ? deals.map(dealCard).join("") : "<p>Fresh deals are being added. Try another filter or check back soon.</p>"}</div></main>
